...
class BookController extends Controller
{
    ...
    public function reservation(Request $request)
    {
        ...
        if ($validate->fails()) {
            ...
            $room = Bedroom::find($input['room_id']);
            $check_in_date = Carbon::createFromFormat('Y-m-d', $input['check_in_date']);
            $duration = $input['duration'];
            $subtotal = 0;
            $pricing = [];
            for ($i = 1; $i <= $duration; $i++) {
                $subtotalNow = $subtotal + $room->allotment()->harga;
                $pricing[$i]['price'] = $room->allotment()->harga;
    ...
    public function book($slug = '')
    {
        ...
        $data['room'] = Bedroom::find($data['reservation']['room_id']);
    ...
    public function snap()
    {
        ...
        $data['room'] = Bedroom::find($reservation['room_id']);
    ...
    public function token()
    {
        ...
        $room = Bedroom::find($reservation['room_id']);
...
//        $room = Bedroom::find($reservation['room_id']);
        ...
        Reservation::insertToJava($reservation, $room->tipe_kamar, $booking);


//        Booking::insertToJava($booking, $id);
