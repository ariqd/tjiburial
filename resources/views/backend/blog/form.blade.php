            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-6">
                                <h1>{{ @$edit == false ? 'Add' : 'Edit' }} Blog</h1>
                            </div>
                            <div class="col-lg-6">
                                <a href="{{ url('admin/blog') }}" class="btn btn-outline-danger float-right">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <form action="{{ !@$edit ? url('admin/blog') : url('admin/blog/'.$blog->id) }}"
                                      method="post">
                                    {{ csrf_field() }}
                                    {{ @$edit ? method_field('PUT') : '' }}

                                    <div class="form-group">
                                        <label for="title" class="bmd-label-floating">Blog Title <span class="required">*</span></label>
                                        <input type="text" name="title" id="title" class="form-control"
                                               value="{{ @old('title') ? old('title') : (@$edit ? $blog->title : '') }}" required>
                                    </div>

                                    <div class="form-group description">
                                        <label for="description" class="bmd-label-floating strong-label">Description <span class="required">*</span></label>
                                        <textarea name="description" id="description" class="summernote">{{ @old('description') ? old('description') : (@$edit ? $blog->description : '') }}</textarea>
                                    </div>
