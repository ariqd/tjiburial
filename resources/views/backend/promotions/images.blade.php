            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-lg-6">
                                <h1>Images of {{ $promotion->title }}</h1>
                            </div>
                            <div class="col-lg-6">
                        ...
                        <div class="row mt-2">
                            <div class="col-lg-12">
                                <form action="{{ url('admin/promotions/images') }}" method="post" enctype="multipart/form-data">
                                    ...
                                    @foreach($promotionImages as $key => $promotionImage)
                                        <div class="promotionImages" id="promotionImages{{ $key }}">
                                            <div class="form-group">
                                                <h3>
                                                    {{ ($key == 0) ? 'Main' : 'Additional' }} Image
                                                    @if($key > 0)
                                                        <button class="btn btn-danger btnDeleteImage pull-right" type="button" data-id="{{ $promotionImage->id }}">
                                                ...
                                                <div class="setting image_picker">
                                                    <div class="settings_wrap">
                                                        <label class="drop_target {{ (@$promotionImage->image && @$promotionImage->image != '') ? 'dropped' : '' }}">
                                                            <div class="image_preview" style="background-image: url('{{ (@$promotionImage->image && @$promotionImage->image != '') ? asset('uploads') . '/promotions/'.$promotion->title.'/'.@$promotionImage->image : '' }}')"></div>
                                                            <input class="inputFile" type="file" name="promotionImages[{{$key}}][image]" data-name="[image]" value="{{ @$promotionImage->image }}"/>
                                                        </label>
                                                        <div class="settings_actions vertical"><a data-action="choose_from_uploaded"><i class="fa fa-picture-o"></i> Drop / choose image to uploads</a><a class="{{ (@$promotionImage->image && @$promotionImage->image != '') ? '' : 'disabled' }}" data-action="remove_current_image"><i class="fa fa-ban"></i> Remove Current Image</a></div>
                                                        <div class="image_details">
                                                            <label class="input_line image_title">
                                                                {{ (@$promotionImage->image && @$promotionImage->image != '') ? @$promotionImage->image : '' }}
                                                            </label>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <input type="hidden" class="deleteImage{{ @$promotionImage->id }}" name="delete[]" value="{{ @$promotionImage->id }}" disabled>
                                    @endforeach
